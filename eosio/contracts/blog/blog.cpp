...
CONTRACT blog : public contract {
    // blog class inherits the eosio “contract” smart contract and uses its
    // constructor below
   ...
   public:
    ...
    blog(name receiver, name code, datastream<const char *> ds)
        : contract(receiver, code, ds), _posts(receiver, receiver.value) {}

    // use ACTION macro so that eosio-cpp will add this as an action to the ABI
    ACTION createpost(const uint64_t timestamp, const name author,
        ...
        // and the transaction will by rolled back - any modifications will be
        // reset
        require_auth(author);

        // generating secondary key (skey) value which is a composite key of the
        // author and the timestamp this allows Demux to also be able to
        // generate and store this key when persisting the record into the
        // database since we cannot return the primary key (pkey), which is
        // generated below (_posts.available_primary_key()), from this action
        ...
        // each action has a new action context which is a clean working memory
        // with no prior working state from other action executions we are
        // adding a record to our table const_iterator emplace( unit64_t payer,
        // Lambda&& constructor )
    ...
    ACTION editpost(const uint64_t timestamp, const name author,
                    const string &title, const string &content,
                    const string &tag) {
   ...
   private:
    // use TABLE macro so that eosio-cpp will add this as a multi_index to the
    // ABI
    TABLE poststruct {
        uint64_t pkey;
        name author;
    ...
    // typedef multi_index<name(table_name), object_template_to_use,
    // other_indices> multi_index_name;
    typedef eosio::multi_index<
        name("poststruct"), poststruct,
        indexed_by<
            name("getbyskey"),
            const_mem_fun<poststruct, uint128_t, &poststruct::get_by_skey> > >
        post_table;
